var GalleryModule = (function($, URLS){
	...
	function sortBinding(el) {
		$('#'+el+'-col').off();
		$('#'+el+'-col').click(function(e) {
			e.preventDefault();
			sortKey = el;
			resetPageParams();
			toggleControls();
	...
	function deleteGalleryClickBinding() {
		$('.deleteGalleryClick').off();
		$('.deleteGalleryClick').click(function(e){
			e.preventDefault();
			...
			data.deleted = true;
			toggleControls();
			$.getJSON(URLS.GALLERY_DELETE(data.galleryid)).done(function(data) {
				applyGalleryTemplate();
				applyPageTemplate();
				bindPagingActions();
				editGalleryClickBinding();
				saveGalleryClickBinding();
				deleteGalleryClickBinding();
				toggleControls();
				$('#success-alert').show();
			}).fail(function() {
	...
	function saveGalleryClickBinding() {
		...
		$('#saveGallery').click(function(e){
			...
			if(matchedGallery.length > 0) {
				...
				if((matchedGallery[0].title==null && matchedGallery[0].title!==$('#title').val().trim()) || matchedGallery[0].title.trim() !== $('#title').val().trim()) {
	...
	function applyLanguageTemplate() {
		...
		$("#language-body").html(renderedHtml);
	}
	...
	function loadGalleries() {
		...
		$.getJSON(URLS.GALLERY(pageIndex, pageSize, selectedCategory, selectedGalleryId ,selectedPartner, selectedTitle,selectedLanguage, sortKey, sortVal)).done(function(data) {
			...
			saveGalleryClickBinding();
			deleteGalleryClickBinding();
			sortBinding('lastupdated');
			sortBinding('creationdate');
			toggleControls();
		}).fail(function() {
